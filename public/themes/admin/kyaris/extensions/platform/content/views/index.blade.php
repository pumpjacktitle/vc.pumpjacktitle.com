...
@section('pageTitle')
	@parent
	: {{ trans('platform/content::general.title') }}
@stop
...
<li class="active">{{{ trans('platform/content::general.title') }}}</li>
@stop
...
<div class="m-b-md">
	<div class="row">
		<div class="col-sm-6">
			<h3 class="m-b-none">{{{ trans('platform/content::general.title') }}}</h3>
		</div>
		<div class="col-sm-6 m-t-md text-right">
			<a class="btn btn-primary" href="{{ URL::toAdmin('content/create') }}"><i class="fa fa-plus"></i> {{{ trans('button.create') }}}</a>
		</div>
	</div>
</div>
	...
	<div class="col-md-12">
		<section class="panel panel-default">
			<div class="row wrapper">
				<div class="col-sm-4">
					...
					<form method="post" action="" accept-charset="utf-8" data-search data-grid="main" class="form-inline" role="form">
						...
						@include('platform/content::grid/loading')
					</form>
				</div>
			...
			<div class="table-responsive">
				<table class="data-grid table table-striped b-t b-light" data-source="{{ URL::toAdmin('content/grid') }}" data-grid="main">
					<thead>
						<tr>
							<th><input type="checkbox" name="checkAll" id="checkAll"></th>
							<th data-sort="name" class="col-md-5 sortable">{{{ trans('platform/content::table.name') }}}</th>
							<th data-sort="slug" class="col-md-2 sortable">{{{ trans('platform/content::table.slug') }}}</th>
							<th data-sort="enabled" class="col-md-2 sortable">{{{ trans('platform/content::table.enabled') }}}</th>
							<th data-sort="created_at" class="col-md-3 sortable">{{{ trans('platform/content::table.created_at') }}}</th>
							<th width="30"></th>
						</tr>
					</thead>
...
@include('platform/content::grid/results')
@include('platform/content::grid/pagination')
@include('platform/content::grid/filters')
@include('platform/content::grid/no_results')
@stop
